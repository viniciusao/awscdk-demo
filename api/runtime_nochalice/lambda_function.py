...
from helpers import init_chars_repository
...
def handler(event, context):
    m = event['requestContext']['http'].get('method')
    c = event.get('pathParameters').get('charname')
    if m == 'GET':
        return get_char_info(c)
    elif m == 'POST':
        return insert_char_info(c)
    elif m == 'DELETE':
        return delete_char_info(c)
